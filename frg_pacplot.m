...
high_anx_subids = [31730];% stai{end-2:end,["ParticipantID"]};
low_anx_subids = [43000];%stai{1:3,["ParticipantID"]};
...
for sc_idx = 1:length(state_conditions)
    ...
    for cat_idx = 1:length(trial_categories)
        ...
        for anx_cat_idx = 1:length(anx_cats)
            ...
            mean_ermi.(sc).(tcat).(anx_cat).ermi = [];
...
%                     for chan_pair_idx = 2%1:length([eeg_dat.etc.eegpac.labels])
%                          [out,ermi,tpts] = frg_plottrialpac(eeg_dat,...
%                                      'plotindx',chan_pair_idx,...
%                                      'freqval1',5.5,'freqval2',35,...
%                                      'timerange',[-500 0]);
%                          C1 = size(out,2);
%                          mean_ermi.(sc).(tcat).(anx_cat).ermi(:,end+1:end+C1) = out;
%                     end
                catch
...
%%
fig_upper = figure('OuterPosition',[73 473 1848 581]); hold on;
...
y1 = mean(mean_ermi.(sc).early.(anx_cat).ermi,2);
y2 = mean(mean_ermi.(sc).mid.(anx_cat).ermi  ,2);
y3 = mean(mean_ermi.(sc).late.(anx_cat).ermi ,2);
y4 = mean(mean_ermi.(sc).leave.(anx_cat).ermi,2);
plot(x,y1,'DisplayName','early');
plot(x,y2,'DisplayName','mid');
plot(x,y3,'DisplayName','late');
plot(x,y4,'DisplayName','leave');

% y1_mean = mean(mean_ermi.(sc).early.(anx_cat).ermi,'all');
% y2_mean = mean(mean_ermi.(sc).mid.(anx_cat).ermi  ,'all');
% y3_mean = mean(mean_ermi.(sc).late.(anx_cat).ermi ,'all');
% y4_mean = mean(mean_ermi.(sc).leave.(anx_cat).ermi,'all');
...
fig_lower = figure('OuterPosition',[1 1 1919 562]); hold on;
